...
func go_query(L *C.lua_State, query *C.go_str) C.int {
	...
	for rows.Next() {
		...
		for i := range row {
			...
			value := C.CString(fmt.Sprintf("%v %T", row[i], row[i])) //@todo(troublete): implement correct value mapping (for postgres)
...
func go_exec(L *C.lua_State, query *C.go_str) C.int {
	...
	C.lua_pushinteger(C.longlong(lastId))
	C.lua_pushinteger(C.longlong(rowsAffected))
	return C.int(2)
}
